...
def change_message(new_message: str, id: int, name: str, version: float) -> Tuple[bool, str]:
    ...
    try:
        with db.open_database(db_path) as conn:
            ...
            rows_updated = db.update_dataset_message(conn, id, version, new_message)
            conn.commit()
            if rows_updated == 0:
            ...
            if rows_updated == 1:
                ...
                return False, f"Multiple datasets found with ID: {id} and version: {version}. Message update may have affected multiple records."
    except sqlite3.Error as e:
